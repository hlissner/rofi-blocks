...
while read -r line; do
	case "$line" in
		"CUSTOM 1" ) sort_column_set "PID";;
		"CUSTOM 2" ) sort_column_set "%CPU";;
		"CUSTOM 3" ) sort_column_set "%MEM";;
		...
		ACCEPT_ENTRY* ) SELECT_ENTRY='"SELECT_ENTRY": 0,'; selected_pid_set "$( tr -s " " <<< "$line" | cut -d" " -f2,2)";;
